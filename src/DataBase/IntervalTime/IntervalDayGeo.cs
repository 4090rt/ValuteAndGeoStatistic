...
{
    public class IntervalDayGeo
    {
        private readonly ILogger<IntervalDayGeo> _logger;
        ...
        public IntervalDayGeo(ILogger<IntervalDayGeo> logger, PoolSQLiteConnect poolSQLiteConnect, SemaphoreSlim semaphore,
        ...
        public async Task<List<RequestGeoLocationSElect>> Request(int page, int pagecount, CancellationToken cancellation = default)
        {
            ...
            try
            {
                ...
                string command = "SELECT * FROM GeoLocations WHERE date(DateUpdate) = date('now') ORDER BY DateUpdate DESC LIMIT @limit OFFSET @offset";
                ...
                await using (var commandsql = new SQLiteCommand(command, connection))
                {
                    ...
                    using (var result = await commandsql.ExecuteReaderAsync())
                    {
                        ...
                        {
                            _logger.LogError("Результат запроса геолокации за день не найден");
                            return default;
                        }
                    }
                }
        ...
        public async Task CreateIdex()
        {
            ...
            try
            {
                ...
                string command = "CREATE INDEX IF NOT EXISTS IX_DayGEO_Date ON GeoLocations(DateUpdate)";
        ...
        public async Task<bool> IndexProverka()
        {
            ...
            try
            {
                ...
                string command = "SELECT COUNT(*) FROM sqlite_master WHERE type = 'index' AND name = 'IX_DayGEO_Date' AND tbl_name = 'GeoLocations'";
                ...
                await using (var commandsql = new SQLiteCommand(command, connection))
                {
                    ...
                    if (resule != null && resule != DBNull.Value)
                    {
                        ...
                        if (exec)
                        {
                            _logger.LogInformation($"✅ Индекс 'IX_DayGEO_Date' существует!");
                        }
                        else
                        {
                            _logger.LogInformation($"❌ Индекс 'IX_DayGEO_Date' не найден");
                        }
                        return exec;
                    }
                    return false;
